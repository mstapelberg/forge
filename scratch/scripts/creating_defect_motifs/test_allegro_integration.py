#!/usr/bin/env python3
"""
Test script to verify Allegro integration with adversarial attacks.
"""
...
def test_allegro_integration():
    """Test Allegro integration with adversarial attacks."""
    
    print("=== Testing Allegro Integration ===")
    ...
    # Example Allegro model path (you'll need to provide a real one)
    model_paths = [
    ...
    # Define chemical symbols mapping to avoid warnings
    chemical_symbols = {
        'Ti': 'Ti',
        'V': 'V', 
        'Cr': 'Cr',
    ...
    try:
        # Initialize optimizer with Allegro
        optimizer = GradientAdversarialOptimizer(
            ...
            debug=True,
            backend='allegro',  # Changed from calculator_type
            species_to_type_name=chemical_symbols  # Use the chemical symbols mapping for Allegro
        )
        
        print(f"Successfully initialized optimizer with {type(optimizer.calculator).__name__} calculator")
        ...
        print(f"Error testing Allegro integration: {e}")
        import traceback
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    success = test_allegro_integration()
    if success:
        print("\n✅ Allegro integration test passed!")
    else:
        print("\n❌ Allegro integration test failed!")
        sys.exit(1) 